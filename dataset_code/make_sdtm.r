...
temp_dm <- tibble(STUDYID = "GSK123456",
                  DOMAIN = "DM",
                  ...
                  ACTARMCD = ARMCD, 
                  ARM = case_when(ARMCD == "A" ~ "GSK123456 100 mg",
                                  ARMCD == "P" ~ "Placebo",
  ...
  group_by(COUNTRY) %>%
  mutate(SUBJID = SUBJID + row_number(),
...
sv<- temp_dm %>% 
  ...
  mutate(SVSTDTC = if_else(ACTARMCD != "SCRNFAIL" | VISITDY < 0,
...
ex <- sv %>% 
  ...
  mutate(EXDOSE = if_else(EXTRT == "Placebo", 0, 100),
         EXDOSEU = "mg",
         DOMAIN = "EX") %>% 
  group_by(USUBJID) %>% 
  ...
  select(STUDYID, DOMAIN, USUBJID, EXSEQ, EXTRT, EXDOSE, EXDOSEU, 
...
vs <- sv %>% 
  select(STUDYID, USUBJID, VSSEQ = SVSEQ, VISIT, VISITNUM, VISITDY, 
         EXSTDTC = SVSTDTC) %>% 
...
ae <- dm %>% 
  ...
  mutate(n_ae = sample(0:5, 200, replace = TRUE, prob = c(0.05,0.225, 0.225, 0.225, 0.225 ,0.05)),
         id = map(n_ae, ~sample(1:nrow(ae_to_sample), ., replace = TRUE))) %>% 
  unnest(id) %>% 
  left_join(ae_to_sample, by = "id") %>% 
  select(-n_ae, -id)%>%
  mutate(AESTDY = sample(0:(total_days-1),  nrow(.), replace = TRUE),
...
all_pp_fmt <- all_pp %>% 
  ...
  pivot_longer(-id) %>% 
  group_by(id) %>% 
  mutate(name = if_else(name %in% paste0("V", 1:5), "FEV1PP", "PEFPP"),
...
re <- vs %>% 
  ...
  distinct() %>% 
  left_join(select(dm , USUBJID, ARMCD,
                   AGE, SEX), 
            by = "USUBJID") %>% 
  mutate(PEFPD = case_when(SEX == "M" ~ 1.0523+0.08272*AGE-0.001301*AGE^2+	0.00024962*HEIGHT^2,
                           SEX == "F" ~ 0.9267+0.06929*AGE-0.001031*AGE^2+0.00018623*HEIGHT^2),
         FEV1PD = case_when(SEX == "M" ~ 0.5536-0.01303*AGE-0.000172*AGE^2+0.00014098*HEIGHT^2,
